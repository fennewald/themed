...
You don't touch the fleet repo — a separate change there will:
1. Add an overlay entry:
   `themed = rustPlatform.buildRustPackage { src = <this repo>; cargoLock.lockFile = ./Cargo.lock; }`
   — so **commit a `Cargo.lock`**.
2. Add `pkgs.themed` to `home.packages` on every host.
3. Define the user service (systemd `--user` / launchd agent) with an `ExecStart`
   roughly:
   ```
   themed \
     --self          <hostname> \
     --listen        <tailscale-ipv4>:<PORT> \
     --state-file    $XDG_STATE_HOME/theme/state.json \
     --socket        $XDG_RUNTIME_DIR/themed.sock \
     --reconcile-cmd '<nushell one-liner, see §5>' \
     --peer fezzik.coelacanth-byzantine.ts.net:<PORT> \
     --peer vizzini.coelacanth-byzantine.ts.net:<PORT> \
     ... (one --peer per other fleet member)
   ```
   `Restart=on-failure` / `KeepAlive=true`. So: **honor these flags**, tolerate
   the tailscale interface not being up yet at start (retry the bind), and make
   `--listen`, `--state-file`, `--socket` overridable. If discovering the local
   tailscale IP is easier from inside the daemon (`tailscale ip -4`) than passing
   `--listen`, propose that back — either is fine, just pick one and document it.
...
`--listen` may be dropped entirely if `tailscale` is on the service's `PATH`.
The CLI wrapper calls `themed --socket <same path> set '<json>'` / `... get`.
